...
extension Resolver: CommonWalletContextProtocol {
    ...
    func createAssetDetails(for assetId: String, in navigation: UINavigationController) throws {
        let navigation = Navigation(navigationController: navigation, style: style)
        self.navigation = navigation

        try prepareAssetDetailsCommand(for: assetId).execute()
    }
