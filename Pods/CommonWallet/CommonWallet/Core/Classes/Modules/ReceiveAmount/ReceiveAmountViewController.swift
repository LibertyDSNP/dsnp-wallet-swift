...
final class ReceiveAmountViewController: UIViewController, AdaptiveDesignable {
    ...
    override func loadView() {
        ...
        shareControl = customViewFactory?.createShareControl()
    ...
    private func configureContentView() {
        ...
        qrView.borderWidth = viewStyle.qrBorderWidth
        qrView.qrBorderView.cornerRadius = viewStyle.qrBorderRadius
        ...
        qrView.widthAnchor.constraint(equalTo: view.widthAnchor).isActive = true

        containerView.stackView.addArrangedSubview(qrSeparatorView)

        qrSeparatorView.widthAnchor.constraint(
            equalTo: view.widthAnchor,
            constant: -2 * Constants.horizontalMargin
        ).isActive = true

        if let shareControl = shareControl {
            shareControl.addTarget(self, action: #selector(actionShare), for: .touchUpInside)
            containerView.stackView.addArrangedSubview(shareControl)
        }
    }

    private func configureNavigationItems() {
        guard shareControl == nil else { return }
